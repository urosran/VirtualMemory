...
import fileSystem.MyFileSystem.MODE;
...
public class MyFileSystem implements FileSystem {
	...
	private DirectBlock getDirectBlock(int fd, MODE mode) {// FIXME!!!
		...
		// of this block
		if (blockNum > 9 && blockNum < (10 + IndirectBlock.COUNT + (IndirectBlock.COUNT * IndirectBlock.COUNT))) {
			...
			if (inode.ptr[11] == 0) {
				...
				// if the inode is empty
					...
					// repeat the upper process for the second level node
					if (freeSpace2 == 0) {
						// if there is no space
						return null;
					} else {
						// TODO: why is index1 used here? - to check for the second free space?
					...
					// TODO: WHY DO I HAVE 3rd space here?
					if (freeSpace3 == 0) {
						return null;
					} else {
					...
					// save the new map with taken spaces
					freeMap.save();
			...
			// if there is a block
				...
				if (fresh) {
					...
					//freshes else ????????????OVDE SI ---> vracam se na vrh da kopiram sve jer je isto
				} else {
					IndirectBlock indirectBlock2 = new IndirectBlock();
		...
		if (blockNum > 9 && blockNum < ((10 + IndirectBlock.COUNT + (IndirectBlock.COUNT * IndirectBlock.COUNT)
			...
			// checking if there is a hole
			// obtain the spot of the node TODO: ASK WHY
			int index1 = (blockNum - ((10 + IndirectBlock.COUNT)) / IndirectBlock.COUNT)/ IndirectBlock.COUNT;
			int index2 = (blockNum - (10 + IndirectBlock.COUNT)) / IndirectBlock.COUNT ;
			// TODO ASK WHY are we moding
			...
			if (inode.ptr[12] == 0) {
				...
				if (fresh) {
					if (mode == MODE.r) {
						...
						// second block from inode to the level 2
						if (freeSpace2 == 0) {
							return null;
						} else {
							indirectBlock2.ptr[index2] = freeSpace2;
							disk.write(indirectBlock.ptr[index2], indirectBlock2);
						}
						// third block from level 2 to the level 3
						if (freeSpace3 == 0) {
							return null;
						} else {
							indirectBlock3.ptr[index3] = freeSpace3;
							disk.write(indirectBlock2.ptr[index2], indirectBlock3);
						}
						// from level 3 to the direct block at level 4
						freeMap.save();
						return new DirectBlock(disk, indirectBlock.ptr[index3], blockOff, false);
						// finished creating when all of them were empty
					}
				} else {
					...
					if (fresh2) {
						if (mode == MODE.r) {
							...
							return new DirectBlock(disk, indirectBlock2.ptr[index3], blockOff, fresh2);
						}
					} else {
						IndirectBlock indirectBlock3 = new IndirectBlock();
